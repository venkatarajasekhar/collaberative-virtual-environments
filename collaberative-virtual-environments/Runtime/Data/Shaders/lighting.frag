...
float ReliefMapping_RayIntersection(in vec2 A, in vec2 AB)
{
	...
	for(int i=0; i<num_steps_lin-1; i++){
		...
		if (depth >= h) { // h is in the heightmap
	...
	// the intersection is between (depth) and (depth - step)
	// So we take (depth - step / 2) to begin
...
{	
	...
	// there is the normal disturbed in space TBN
...
//// Lighting :
	vec4 base = texture2D(texDiffuse, uv);	// Color diffuse

	float iDiffuse = max(dot(L, N), 0.0);	// Intesity diffuse
	...
	// If it's a spot light
	if(vLightTBN.w > 1.5)
	{
		// Create spotlight halo
...
float ShadowMapping(vec4 vVertexFromLightView, out vec3 vPixPosInDepthMap)
{
	...
	vPixPosInDepthMap = vVertexFromLightView.xyz/vVertexFromLightView.w;	// homogenization
	vPixPosInDepthMap = (vPixPosInDepthMap + 1.0) * 0.5;					// The interval [-1 1] to [0 1]
	...
	if((vDepthMapColor.z+Z_TEST_SIGMA) < vPixPosInDepthMap.z)
	{
		...
		// Soft Shadow for fragments close
		if( length(vVertexMV.xyz) < 12.0 )   // < 12 
		{
			for(int i=0; i<9; i++)
			{
				vec2 offset = tOffset[i] / (float(depth_map_size));
				// Pixel color depth on the map
