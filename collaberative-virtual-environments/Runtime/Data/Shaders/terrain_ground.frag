...
void main (void)
{
	// Clip plane if rendering water reflection
...
//// LIGHTING :
	...
	float iDiffuse = max(dot(lightVecTBN.xyz, normalTBN), 0.0);	// Diffuse intensity
	float iSpecular = 0;
...
*/
	...
	for(int i=0; i<2; i++) 
	{
		if(!ok)
		{
			...
			vPixPosInDepthMap = (vPixPosInDepthMap + 1.0) * 0.5;					// between [-1 1] to [0 1]
