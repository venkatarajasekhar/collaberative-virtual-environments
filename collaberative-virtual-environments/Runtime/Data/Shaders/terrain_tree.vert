...
void main(void)
{
	...
	// Animate trees
	float move_speed = (float(int(vertexM.y*vertexM.z) % 50)/50.0 + 0.5);    // 50)/50.0
	float move_offset = vertexM.x;
	vertex.x += 0.01 * pow(vertex.y, 1.5) * cos(time * move_speed + move_offset);  // Pow( y pos, strength )
	
	
	vec4 vLightPosMV = -gl_LightSource[0].position;		// Directional light
