...
bool Mesh::Load(const std::string& name)
{
	...
	// Set up path for mesh
	char szPath[MAX_PATH+1];

	GetCurrentDirectory(MAX_PATH, szPath);				// Get Our Working Directory
	strcat_s(szPath, "\\Data\\Meshs\\");				// Append "\\Data\\Meshs\\" After The Working Directory
	...
	if(!ret) {
		std::cerr << "[Error] Unable to load the mesh: " << name << std::endl;
		return false;
	}
...
bool Mesh::LoadOBJ(const std::string& filename)	// Load object (Wavefront 3D .obj)
{
	...
	vec2 pt2D;
//	char buftmp[64];
	long i,v,g;				// Counter for storing data during second pass
...
//	grp.Actif3D = true;
	...
	//		sscanf(buf,"%s",buftmp);
			...
			strStream >> buftmp;
		//	buftmp << strStream;
			if(buftmp == "#") {							// If we found a comment, ignore it
			} else if(buftmp == "mtllib") {				// Is their a material to load?
	//			materials->Load(&buf[7]);				// Loading file assosciated
			} else if(buftmp == "v") {					// We found a vertex
				sscanf(&strBuffer[2],"%f%f%f",&pt3D.x,&pt3D.y,&pt3D.z);
				tTempPosition.push_back(pt3D);
			} else if(buftmp == "vt") {					// A texture coord
				sscanf(&strBuffer[2],"%f%f",&pt2D.x,&pt2D.y);
				pt2D.t = 1.0f - pt2D.t;
				tTempTexcoord.push_back(pt2D);
			} else if(buftmp == "vn") {					// We found a normal
				sscanf(&strBuffer[2],"%f%f%f",&pt3D.x,&pt3D.y,&pt3D.z);
				pt3D.normalize();
				tTempNormal.push_back(pt3D);
			} else if(buftmp == "g") {					// We found a group
				if(strBuffer.size() > 1) {
					grp.strName = &buf[2];
					grp.nMaterial = 0;
					m_tGroup.push_back(grp);
				}
			} else if(buftmp == "usemtl") {				// If a material has to be used
				if(m_tGroup.size() <= 0) {
					grp.strName = "No Name";
					grp.nMaterial = 0;
					m_tGroup.push_back(grp);
...
//				m_tGroup[g].Material = materials->GetId(&buf[7]);	// recovering its id
			} else if(buftmp == "f") {					// If we found a face?
		...
		//		m_tTexcoord.resize( m_tPosition.size() );
				...
				for(i=0; (buf[i] < '0') || (buf[i] > '9') ;i++);		// It is positioned at the first value
				for(v=0; v < 3 ;v++) {									// triangles, 3 vertices
					indFacePosition = 0;
					for(; (buf[i] >= '0') && (buf[i] <= '9') ;i++) {	// it is recovered
						indFacePosition *= 10;							// the first vertex
						indFacePosition += buf[i]-0x30;					// 0x30 ASCII value for'0'
					}
					indFacePosition--;									// index is not between 1 and nbFaces but from 0 and nbFaces-1
					NEXT_INDICE;										// go to next indice
					indFaceTexcoord = 0;
					for(; (buf[i] >= '0') && (buf[i] <= '9') ;i++) {	// It is recovered
						indFaceTexcoord *= 10;							// next text coord
						indFaceTexcoord += buf[i]-0x30;
					}
					indFaceTexcoord--;									// index is not between 1 and nbFaces but from 0 and nbFaces-1
					NEXT_INDICE;										// etc... their are 9 indices
					indFaceNormal = 0;
					for(; (buf[i] >= '0') && (buf[i] <= '9') ;i++) {
						indFaceNormal *= 10;							// first normal
						indFaceNormal += buf[i]-0x30;
					}
					indFaceNormal--;									// index is not between 1 and nbFaces but from 0 and nbFaces-1
					if(v < 2) NEXT_INDICE;
...
*/
				...
				m_tGroup[g].tFace.push_back(face);						// save the recovered face
			}
		}
//		delete[] buf;
	}
