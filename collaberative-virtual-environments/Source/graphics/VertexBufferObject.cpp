...
/*
- GL_STREAM_DRAW when vertices data can be updated between each render.
- GL_DYNAMIC_DRAW when vertices data can be updated between each frame.
...
{
	if(m_nVBOid)	Enable_VBO();		// with Vertex Buffer Object
	else			Enable_VA();		// with Vertex Array
}

void VertexBufferObject::Disable()
{
	if(m_nVBOid)	Disable_VBO();		// with Vertex Buffer Object
	else			Disable_VA();		// with Vertex Array
}
