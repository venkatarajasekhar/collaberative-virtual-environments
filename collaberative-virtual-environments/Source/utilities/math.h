...
#define PI					3.141592653589793238462643383279f		// PI
...
struct axis
{
	...
	vec3d x, y, z;
	axis() { defaultInit(); }
	axis(quaternion &q){defaultInit(); x=q.rotate(x); y=q.rotate(y); z=q.rotate(z); normalize();}
	axis(vec3d &o){defaultInit(); origin=o;}
	axis(vec3d &o, quaternion &q)
	{
		defaultInit();
		x=q.rotate(x);
		y=q.rotate(y);
	...
	axis(const axis &a){x=a.x;y=a.y;z=a.z;origin=a.origin;}
	
	void normalize(){x.normalize(); y.normalize(); z.normalize();}

	axis &operator =(axis &a){x=a.x;y=a.y;z=a.z;origin=a.origin;return *this;}

	vec3d toLocal(vec3d &input)
	{
		vec3d res;
		res=vec3d((input-origin).dot(x), (input-origin).dot(y), (input-origin).dot(z));
		return res;
	}

	vec3d toGlobal(vec3d &input)
	{
		vec3d res;
		res=x*input.x+y*input.y+z*input.z;
		res+=origin;
		return res;
	}
	private:
	void inline defaultInit(){x=vec3d(1, 0, 0); y=vec3d(0, 1, 0); z=vec3d(0, 0, 1);}
};
...
class matrix
{
	...
	public:
		float *dta;
		inline float &operator [](int col){return dta[col];}
	};
	matrixRow rows[4];
public:
	matrix(){rows[0].dta=data[0]; rows[1].dta=data[1]; rows[2].dta=data[2];rows[3].dta=data[3];
	data[0][0]=data[1][1]=data[2][2]=data[3][3]=1.0f;
	data[1][0]=data[2][0]=data[3][0]=0.0f;
	data[0][1]=data[2][1]=data[3][1]=0.0f;
	data[0][2]=data[1][2]=data[3][2]=0.0f;
	data[0][3]=data[1][3]=data[2][3]=0.0f;}

	float data[4][4];
	matrixRow &operator [](int row){return rows[row];}
	...
	{
		data[1][1]=(float)cos(radians);
		data[2][1]=(float)sin(radians);
		data[1][2]=(float)-sin(radians);
		data[2][2]=(float)cos(radians);
	}
	void rotateY(float radians)
	{
		data[0][0]=(float)cos(radians);
		data[2][0]=(float)-sin(radians);
		data[0][2]=(float)sin(radians);
		data[2][2]=(float)cos(radians);
	}
	void rotateZ(float radians)
	{
		data[0][0]=(float)cos(radians);
		data[1][0]=(float)sin(radians);
		data[0][1]=(float)-sin(radians);
		data[1][1]=(float)cos(radians);
	}
	void followedBy(matrix &m)
	{
		matrix res;
		for(int i=0;i<3;i++)
		{
			for(int j=0;j<3;j++)
			{
				res[i][j]=data[i][0]*m[0][j] + data[i][1]*m[1][j]+data[i][2]*m[2][j];
				//res[i][j]=data[i][0]*data[0][j] + m[i][1]*data[1][j]+m[i][2]*data[2][j];
			}
		}
		memcpy(data, res.data, sizeof(float)*4*4);
	}
	
	void getYPR(float &y, float &p, float &r)
	{
		y=(float)atan(data[1][0]/data[0][0]);
		p=(float)asin(-data[2][0]);
		r=(float)atan(data[2][1]/data[2][2]);
	}
};
...
public:
	...
	vec2d apply(matrix &v)
	{
		vec2d res;
		res.x= x*v[0][0] + y*v[1][0] + v[3][0];
		res.y= x*v[0][1] + y*v[1][1] + v[3][1];
		return res;
	}
...
public:
	...
	vec3d apply(matrix &v)
	{
		vec3d res;
		res.x= x*v[0][0] + y*v[1][0] + z*v[2][0]+v[3][0];
		res.y= x*v[0][1] + y*v[1][1] + z*v[2][1]+v[3][1];
		res.z= x*v[0][2] + y*v[1][2] + z*v[2][2]+v[3][2];
		return res;
	}
...
int inline nextPowerOfTwo(int x)
{
